...
class GuardianState:
    """守护状态"""
    last_actions: Dict[str, float] = field(default_factory=dict)  # 场景ID → 最后触发时间
    cooldown: float = 1800.0  # 冷却时间（秒）= 30 分钟
    total_cares: int = 0      # 总关怀次数
    last_mood: str = "平静"   # 检测到的最后情绪
...
class GuardianSystem:
    ...
    def evaluate(self, user_input: str, sentiment: Optional[Dict] = None) -> Optional[Dict]:
        """评估用户输入，返回匹配的守护场景"""
        for scenario in sorted(GUARDIAN_SCENARIOS, key=lambda x: -x["priority"]):
            # 检查冷却
            last_time = self.state.last_actions.get(scenario["id"], 0)
            ...
            for keyword in scenario["keywords"]:
                if keyword in user_input:
                    ...
                    self.state.last_actions[scenario["id"]] = time.time()
                    self.state.total_cares += 1
                    self.state.last_mood = scenario["name"]
                    self._save()

                    logger.info(f"守护触发 [{scenario['name']}]: {response[:30]}...")
                    return {
