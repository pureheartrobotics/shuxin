...
- **百度地图 MCP（平台托管）**：**默认关闭**（成本；`MapConfig.enabled=False` / compose `SHUXIN_MAP_TOOLS_ENABLED` 默认 `0`）。**服务器启停只改 `.env` 的 `SHUXIN_MAP_TOOLS_ENABLED` + `SHUXIN_BAIDU_MAP_AK`，再 `bash scripts/redeploy_docker.sh --skip-build`；勿改源码里 `MapConfig.enabled` 默认值（仅为 env 未设时的兜底）。** 代码仍在 `integrations/location/`（`BaiduMcpLocationProvider`，Streamable HTTP）。关闭时不注册 tools、不打 MCP；`user_profile.json` 的静态 location 仍可注入 Slot4。开启后：localhost 私网 IP 须用出口公网（`ip_resolve.py`）或 `SHUXIN_MAP_DEFAULT_REGION`；门控命中后才附 tools；模型不调 tool 时天气/POI 服务端直调降级。Voice 默认 `SHUXIN_MAP_MAX_TOOL_ROUNDS=1`、`SHUXIN_MAP_MCP_TIMEOUT_SECONDS=3`。`map_ip_location` 仅服务端静默解析，不暴露给 LLM。tool 轮 Voice 发 `agent/thinking`（`reason: map_lookup`）。
...
- **设备真人通话（百度 Pure RTC）**：真听声主路径仅 **Web↔Web**（`/voice-demo` + `baidu.rtc.sdk.js`）；舒心只发 `call/*` 信令与 token（`BAIDU_RTC_*`，`SHUXIN_RTC_CALL_ENABLED`），**媒体不经舒心转发**。量产板 **ESP32-C3** 当前仅信令/铃声（`BrtcClient` stub）；**禁止**假定硬件已能互听，或把阿里/腾讯手机 SDK、ASR3601 FreeRTOS 文档、大模型互动 RTOS 当作 C3 P2P 方案。Web 验收 [`docs/RTC_WEB_CALL_LAB.md`](docs/RTC_WEB_CALL_LAB.md)；规格/带宽口径 [`docs/RTC_DEVICE_CALL_SPEC.md`](docs/RTC_DEVICE_CALL_SPEC.md)；硬件振铃 lab [`docs/RTC_HARDWARE_CALLEE_LAB.md`](docs/RTC_HARDWARE_CALLEE_LAB.md)。
...
- **量产入口**：TabBar **设备 | 商城 | 我的**（`modules/mall`）；配网 `SHOW_PROVISION_DEV_LOGS=false`。商城运营见 `/miniapp-admin`「商城管理」。恢复/架构见 [`docs/MALL_MODULE_ARCHITECTURE.md`](docs/MALL_MODULE_ARCHITECTURE.md)、[`docs/WECHAT_MINIPROGRAM.md`](docs/WECHAT_MINIPROGRAM.md) §6。
