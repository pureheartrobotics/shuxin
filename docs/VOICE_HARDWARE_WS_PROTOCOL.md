...
固件收到 `error_kind == "quota_exhausted"` 后应播放本地预置提示音（资源键 `QUOTA_EXHAUSTED`，本地文案可仍为「额度已用尽，请充值」），**不要**等待云端 TTS。WebSocket 连接保持，用户充值后可继续对话。服务端用户向 `message` 现为「陪伴点已用尽，请充值」；固件宜以 `error_kind` 为准，亦可兼容旧串「额度已用尽」。
